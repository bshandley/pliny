...
export default function AdminPage({ onBack, currentUser, subRoute, onNavigate }: AdminPageProps) {
  ...
  const activeLabel = TABS.find(t => t.key === activeTab)?.label ?? 'Members';
    ...
    <div className="admin-page">
      ...
      <div className="admin-layout">
        ...
        <div className="admin-mobile-nav" ref={dropdownRef}>
          ...
          >
            <span className="admin-tab-icon">{TAB_ICONS[activeTab]}</span>
            <span>{activeLabel}</span>
            <svg className={`admin-dropdown-chevron${dropdownOpen ? ' open' : ''}`} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          {dropdownOpen && (
            <div className="admin-dropdown-menu">
              {TABS.map(tab => (
                <button
                  key={tab.key}
                  className={`admin-dropdown-item${activeTab === tab.key ? ' active' : ''}`}
                  onClick={() => { onNavigate(tab.route); setDropdownOpen(false); }}
                >
                  <span className="admin-tab-icon">{TAB_ICONS[tab.key]}</span>
                  <span>{tab.label}</span>
                </button>
