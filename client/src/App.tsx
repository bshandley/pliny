...
function App() {
  ...
  const [theme, setTheme] = useState<'light' | 'dark' | 'pliny'>(() => {
    const saved = localStorage.getItem('theme');
    return (saved as 'light' | 'dark' | 'pliny') || 'pliny';
  });
  ...
  const toggleTheme = async (e?: React.MouseEvent) => {
    ...
    if (
      ...
      setTheme(prev => prev === 'light' ? 'dark' : prev === 'dark' ? 'pliny' : 'light');
      return;
    }
    ...
    return (
      <>
        ...
        <button
          ...
          title={`Switch to ${theme === 'light' ? 'dark' : theme === 'dark' ? 'Pliny' : 'light'} mode`}
        >
